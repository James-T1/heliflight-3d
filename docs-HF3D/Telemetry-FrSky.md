# FrSky Telemetry setup for Heliflight 3D

The best way to setup Telemetry is to have the on-board "Vbat" ADC monitoring monitor the voltage that the flight controller is operating off of.
	* For really small helicopters (<3S) where the servos are ran from the flight controller's on-board BEC, Vbat will be the full pack voltage.
	* For larger helicopters where there is a separate BEC (either inside the ESC or not), then Vbat will be the BEC output voltage.
	* For larger helicopters using a receiver pack (usually a 2S LiPo), then Vbat will be the LiPo pack voltage.
	* Best to probably disable the "per cell volts" option in Heliflight.

You can also wire the "Telemetry" or "TX" pin on a DSHOT ESC to a UART_RX port on the flight controller.  This will give you data from the ESC that includes:  main pack voltage, ESC current draw, ESC temperature, and motor RPM.
	* This data will not be used by Heliflight to trigger anything internally (such as low voltage cutoff), as that is driven from the "Vbat" field.
		○ You can setup Vbat to be assigned to the ESC telemetry sensor, but then you will not have any monitoring of the BEC or RX pack voltage.
	* The ESC telemetry data will be automatically forwarded to the transmitter over the FrSky Fport/Sport telemetry using new sensors that you can discover from the model's Telemetry menu in OpenTX.

Enable ESC telemetry sensors
	* set telemetry_disabled_esc_voltage = false
    * 

Setup your alarms in OpenTX as follows:
	* VFAS = Vbat from Heliflight (BEC or receiver pack voltage)
	* VFAS1 = ESC 1 main pack voltage
	* VFAS2 = ESC 2 main pack voltage
		** Only for dual-motor helis with two ESCs
		** Should be same as ESC 1 main pack voltage

Ensure that you setup the "Voltage Meter" parameters in the configuration tab so that the Vbat voltage reported in Configurator matches the voltage you find with a voltmeter (for a BEC) or cell checker plugged into the balance port of the battery (for a battery).


This telemetry data transmission to the TX can be found in /src/main/telemetry/smartport.c in the "processSmartPortTelemetry" function.  This function is used by both Fport and Sport telemetry.
